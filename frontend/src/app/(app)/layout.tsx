  ...
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        const unsubscribeProfile = onSnapshot(userRef, (snap) => {
          if (snap.exists()) {
            ...
            if (!data.active_semester) {
              ...
              // Task 4: Semester Transition Protocol
              const endDate = new Date(data.active_semester.semester_end_date);
              if (new Date() > endDate) {
        ...
        // Redir to auth if not logged in (handled by next middleware or simple check)
      }
    });
