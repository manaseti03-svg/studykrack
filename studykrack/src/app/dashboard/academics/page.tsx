      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          ...
          <form onSubmit={addRecord} className="flex gap-4 items-end">
            ...
            <button type="submit" className="px-6 py-2 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 h-[42px] flex items-center justify-center">
          ...
          <ul className="divide-y divide-slate-100">
            {records.map(record => (
              <li key={record.id} className="p-4 grid grid-cols-4 items-center hover:bg-slate-50 transition">
                ...
                <div className="text-slate-600">{record.score} / {record.total} <span className="text-sm text-slate-400 ml-2">({((record.score / record.total) * 100).toFixed(1)}%)</span></div>
