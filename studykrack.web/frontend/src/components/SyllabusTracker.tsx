...
export default function SyllabusTracker() {
  ...
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      ...
      // Stage 1: Real-time Cloud Sync with Firestore (Strictly filtered by owner_id)
      const userRef = doc(db, "users", user.uid);
      const unsubscribe = onSnapshot(userRef, (snap) => {
        if (snap.exists()) {
          const userData = snap.data();
          setProfile(userData);
          const subjects = userData.active_semester?.subjects || [];
          
          if (subjects.length > 0) {
            const units = subjects.map((subj: string, idx: number) => ({
              id: idx + 1,
              name: subj,
              is_uploaded: false // Logic for upload status could be refined later
            }));
            
            // Basic logic to show progress based on vault count for now
            // In a real scenario, we'd check if specific subjects have been researched
            setData(units);
          } else {
            // Fallback to empty or placeholders if no subjects
            setData([]);
          }
        }
      }, (error) => {
        console.error("Tracker Sync Denied:", error);
      });
      ...
      <div className="bg-white/5 backdrop-blur-2xl p-10 rounded-[40px] shadow-2xl relative overflow-hidden border border-white/10">
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-1">
            <h3 className="text-2xl font-headline font-bold text-white tracking-tight">{profile?.name || "Anonymous"}</h3>
            <p className="font-label text-primary font-bold text-[10px] tracking-[0.4em] uppercase">{profile?.school || "My School"}</p>
            <p className="font-label text-white/40 font-medium text-[9px] uppercase tracking-widest">{profile?.branch || "General"} | {profile?.semester || "Sem 1"}</p>
          </div>
