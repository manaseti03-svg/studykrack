...
export default function OnboardingWizard({ onComplete }: { onComplete: () => void }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/vision/timetable`, {
        method: "POST",
        body: formData,
      });
      ...
      console.error(err);
      const useManual = confirm("Vision Analysis Engine is currently under maintenance. Would you like to enter your details manually?");
      if (useManual) {
        setMatrix({
          timetable: [{ day: "Monday", subject: "Mathematics", time: "9:00 AM" }],
