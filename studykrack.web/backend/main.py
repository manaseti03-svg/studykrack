...
try:
    ...
    if os.path.exists(key_path):
        ...
        # Fallback to default auth if project_id exists but no keys
        firebase_admin.initialize_app(options={
            'storageBucket': storage_bucket,
            'projectId': project_id
        })
    db = firestore.client(database_id=os.getenv("FIREBASE_DATABASE_ID"))
    print(f"[SYSTEM] Firebase Admin & Storage Bucket {storage_bucket} Connected to Database {os.getenv('FIREBASE_DATABASE_ID')}.")
except Exception as e:
    print(f"[WARNING] Firebase Admin Failure: {e}. Switching to LOCAL_RESILIENCE_MODE (No DB).")
    db = None
...
def check_user_quota(uid: str = "muni_manas_01"):
    ...
    fuel_balance = user_data.get("fuel_balance", 0)
    ...
    if status == "fuel" and fuel_balance <= 0:
        raise HTTPException(status_code=402, detail="Zero-Trust Block: Out of Fuel. Please upgrade to continue.")

    if status == "fuel":
        user_data["fuel_balance"] = fuel_balance - 1
    else:
...
async def pulse_radar_search(search_query: SearchQuery):
    if not db: 
        print("[CRITICAL] Firestore DB not initialized. Check serviceAccountKey.json.")
        return {
            "status": "maintenance",
            "results": [{
            "id": "db-offline-mock",
    ...
    try:
        ...
        if best_score < 0.85 or search_query.deep_research:
            ...
            print("[ROUTER] Low vault relevancy (Score: {:.2f}). Igniting Path B: Global Sentinel Fallback...".format(best_score))
            if not verify_execution_token(search_query.execution_token, search_query.execution_payload):
